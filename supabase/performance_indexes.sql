...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_prospect_id_jobs 
ON scrape_prospect(id_jobs);

-- Partial index on email (only non-null emails)
-- Used for: email filtering, verification
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_prospect_email 
ON scrape_prospect(email) 
WHERE email IS NOT NULL;

-- Partial index on ville (only non-null cities)
-- Used for: location-based searches
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_prospect_ville 
ON scrape_prospect(ville) 
WHERE ville IS NOT NULL;

-- Composite index for common query patterns
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_prospect_job_created 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_jobs_user_created 
ON scrape_jobs(id_user, created_at DESC);

-- Index on status for filtering
-- Used in: ScrapingProgressWidget, SearchHistoryTable
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_jobs_status 
ON scrape_jobs(statut);

-- Composite for user + status queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_scrape_jobs_user_status 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_gen_campaign 
ON email_generations(campaign_id);

-- Index on status for filtering
-- Used for: filtering by sent/opened/replied
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_gen_status 
ON email_generations(status);

-- Composite for campaign listings with date
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_gen_campaign_created 
ON email_generations(campaign_id, created_at DESC);

-- Partial index for tracking sent emails
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_gen_sent 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_verif_jobs_user 
...
-- Index on job FK
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_email_verif_results_job 
